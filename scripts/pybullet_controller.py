...
class RobotController:
    ...
    def createWorld(self, GUI=True, view_world=False):
        ...
        self.robot_id = p.loadURDF("/home/annamalai/geo_us/src/ros_geomagic/geomagic_description/urdf/ur5.urdf", useFixedBase=True)
    ...
    def setJointPosition(self, position, kp=1.0, kv=1.0):
        ...
        for i in range(10): #!!! to settle the robot to its position we can reduce it !!!
            p.stepSimulation()        
            (jointPosition,jointVelocity,jointReactionForces,appliedJointMotorTorque) = p.getJointState(self.robot_id, self.end_eff_index)
            print(jointReactionForces[0],jointReactionForces[1],jointReactionForces[2])
            of.force.x = min(max(jointReactionForces[0]/400000,-1),1)
            of.force.y = min(max(jointReactionForces[1]/400000,-1),1)
            of.force.z = min(max(jointReactionForces[2]/400000,-1),1)
            of.lock = [False, False, False]
            pub.publish(of)
            msg = Wrench()
            msg.force.x = jointReactionForces[0]
            msg.force.y = jointReactionForces[1]
            msg.force.z = jointReactionForces[2]
            pub_mes.publish(msg)
